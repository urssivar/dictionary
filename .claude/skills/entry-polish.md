...
- `scripts/validate.py` handles mechanical checks (tags, schema, IDs, folder); don't re-check those — focus on judgment calls: alias POS, example quality, note clarity, semantic tag fitness, etymology category pairing (`en`/`ru` naming the same grammatical category, e.g. participle vs converb)
...
- Deterministic etymology checks `validate.py` owns — transliteration by source, glossing rules, term/hedge vocabulary, terminal punctuation
